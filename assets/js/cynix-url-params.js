...
(function() {
    ...
    function isBrandLogoImage(img) {
        ...
        var brandContext = img.closest(
            ...
            '.footer-logo, .footer-widget .footer-logo'
        );
    ...
    function applyFooterBranding() {
        var footers = document.querySelectorAll('.footer-wrapper');
        var phoneHref = 'tel:' + phone.replace(/\s/g, '').replace(/[^\d+]/g, '');

        footers.forEach(function(footer) {
            var footerTitle = footer.querySelector('.widget.footer-widget .widget_title');
            if (footerTitle && normalizeText(footerTitle.textContent) === 'stay in touch') {
                footerTitle.textContent = 'Get In Touch';
            }

            var infoBlocks = footer.querySelectorAll('.footer-info');
            if (infoBlocks[0]) {
                infoBlocks[0].innerHTML = '<i class="fa-regular fa-location-dot"></i>' + address;
            }

            if (infoBlocks[1]) {
                infoBlocks[1].innerHTML = '<i class="fa-sharp fa-solid fa-phone"></i>' +
                    '<span>' +
                    '<a class="text-inherit d-block" href="' + phoneHref + '">' + phone + '</a>' +
                    '<a class="text-inherit d-block" href="' + phoneHref + '">' + phone + '</a>' +
                    '</span>';
            }

            if (infoBlocks[2]) {
                infoBlocks[2].innerHTML = '<i class="fa-sharp fa-solid fa-envelope"></i>' +
                    '<span>' +
                    '<a class="text-inherit d-block" href="mailto:' + email + '">' + email + '</a>' +
                    '<a class="text-inherit d-block" href="mailto:support@cynix.inc">support@cynix.inc</a>' +
                    '</span>';
            }

            var footerBrandLink = footer.querySelector('.footer-link');
            if (footerBrandLink && normalizeText(footerBrandLink.textContent).indexOf('faren') !== -1) {
                footerBrandLink.textContent = '@cynixinc';
            }

            var copyright = footer.querySelector('.copyright-text');
            if (copyright) {
                copyright.innerHTML = 'Copyright <i class="fal fa-copyright"></i> 2026 <a href="#">Cynix Inc</a>. All Rights Reserved.';
            }
        });
    }
